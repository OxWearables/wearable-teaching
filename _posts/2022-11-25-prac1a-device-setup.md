...
Start by installing the wearable camera browser to your home folder. You can find more details about the camera browser on [GitHub](https://github.com/activityMonitoring/oxford-wearable-camera-browser). Note, this browser is currently in the process of being updated, so do not be surprised if there are dependency warnings in this legacy version. 

First, if you have not done so already, install [homebrew](https://brew.sh):
```shell
$ /bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)"
```
Next, you want to make sure you have node.js installed on your iMac:
```shell 
$ brew install node
```
...
$ npm install  
```
Finally, you can run the camera browser as follows, (though we will only use it this afternoon):
```
$ npm start
```

We now turn to setting up your wearable camera. In your terminal, run the following commands:

Within the oxford-wearable-camera-browser directory, navigate to the `utilities` folder. We will use the script `autographer.py` located inside this folder. Start a virtual environment and install `tqdm`:
```shell
...
$ pip install tqdm
```
Now plug your camera into your Mac. Assuming you are still in `~/oxford-wearable-camera-browser/utilities/ `, run the following to wipe the camera clean:
```shell 
...
At this point, you should have an accelerometer mounted on your wrist, and a camera worn on a lanyard around your neck. If you feel sufficiently surveilled, call over one of your tutors to check you have done everything correctly, and then try to pack in as many fun activities for the rest of the practical. Later this afternoon, we will annotate these activities. It is also worth taking this time to reflect on the various ethical implications raised by using wearable devices in research. 
